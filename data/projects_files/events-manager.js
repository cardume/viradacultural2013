			...
			if(data.length > 0){
				  ...
				  for (var i = 0; i < data.length; i++) {
					  if( !(data[i].location_latitude == 0 && data[i].location_longitude == 0) ){
						...
						var infobox = em_map_infobox(marker, myContent, maps[map_id], data[i]);
						var obj_info = {data: data[i], marker: marker, infobox: infobox};
						markerData[data[i].location_id] = obj_info;
				...
				jQuery(document).triggerHandler('em_maps_locations_hook', [maps[map_id], markerData, map_id]);
			}else{
...
function em_map_infobox(marker, message, map, data) {
  var iw = new google.maps.InfoWindow({ content: message });
  var newData = {data: data, marker: marker, infobox: iw};
  google.maps.event.addListener(marker, 'click', function() {
	if( infowindow ) infowindow.close();
	infowindow = iw;
    iw.open(map,marker);
    jQuery(document).triggerHandler('marker_click', [newData, true]);
